  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 bg-transparent">
      <div className="max-w-7xl mx-auto flex items-center justify-between px-4 sm:px-6 md:px-12 py-3">
        {/* Logo (small) */}
        <h2 className="text-lg md:text-sm font-semibold text-black">
          Mable <span className="text-[#264E36]">Tusiime</span>
        </h2>

        {/* desktop links: explicitly white by default */}
            ...
            <Link
              ...
              className={`text-sm transition px-2 py-1 rounded ${
                location.pathname === link.path
                  ? 'text-white'
                  : 'text-white hover:text-[#174d36] hover:font-semibold'
              }`}
            >
              {link.name}
            </Link>
          ...
          <div className="flex flex-col items-center gap-4 py-8">
            {navLinks.map((link) => (
              <Link
                ...
                to={link.path}
                onClick={() => setOpen(false)}
                className={`text-lg px-4 py-2 rounded-md w-11/12 text-center ${
                  location.pathname === link.path ? 'text-[#174d36] font-semibold' : 'text-white'
                }`}
              >
                {link.name}
              </Link>
